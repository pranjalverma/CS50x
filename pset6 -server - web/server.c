...
int main(int argc, char* argv[])
{
    ...
    // https://www.gnu.org/software/libc/manual/html_node/Using-Getopt.html (edited comment)
    int opt;
    ...
    // http://pubs.opengroup.org/onlinepubs/009695399/basedefs/signal.h.html (edited comment)
    ...
    // size_t being an unsigned int (edited comment)
    char* message = NULL;
    size_t length = 0;
        ...
        if (connected())
        {
            ...
            if (request(&message, &length))
            {
                ...
                if (parse(line, abs_path, query))
                {
                    ...
                    // struct stat stores info about a file (edited comment)
                    // https://www.gnu.org/software/libc/manual/html_node/Testing-File-Type.html (edited comment)
                    // http://codewiki.wikidot.com/c:struct-stat (edited comment)
                    struct stat sb;
                    ...
                    // look up MIME(Multipurpose Internet Mail Extensions) type for file at path
                    // MIME type e.g. - "text/html" or "image/jpeg" etc (edited comment)
...
char* indexes(const char* path)
{
    if (path != NULL)
    {
        ...
        char* new_pathp = calloc(strlen(path) + strlen(php) + 1, sizeof(char));
        if (new_pathp == NULL)
            return NULL;
            
        strncpy(new_pathp, path, strlen(path));
        new_pathp[strlen(path + 1)] = '\0';
        strncat(new_pathp, php, strlen(php));
        if (access(new_pathp, R_OK) == 0)
            return new_pathp;
        
        char* new_pathh = calloc(strlen(path) + strlen(html) + 1, sizeof(char));
        if (new_pathh == NULL)
            return NULL;
            
        strncpy(new_pathh, path, strlen(path));
        new_pathh[strlen(path + 1)] = '\0';
        strncat(new_pathh, html, strlen(html));
        if (access(new_pathh, R_OK) == 0)
            return new_pathh;
...
void list(const char* path)
{
    ...
    // http://man7.org/linux/man-pages/man2/access.2.html# (edited comment)
    ...
    // https://www.gnu.org/software/libc/manual/html_node/Directory-Entries.html (edited comment)
    // http://man7.org/linux/man-pages/man3/scandir.3.html (edited comment)
    ...
    // prepare response 
    // this block is a work of art (edited comment)
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    BYTE* buffer = malloc(sizeof(BYTE) + 1);
    if (buffer == NULL)
        return false;
    ...
    for (int c = fgetc(file); c != EOF; c = fgetc(file))
    {
        if (bytes != 0)
        {
            buffer = realloc(buffer, bytes + 1 + 1);
            if (buffer == NULL)
                return false;
...
const char* lookup(const char* path)
{
    if (path != NULL)
    {
        if (strcasestr(path, ".css") != NULL)
            return "text/css";
        if (strcasestr(path, ".html") != NULL)
            return "text/html";
        if (strcasestr(path, ".gif") != NULL)
            return "image/gif";
        if (strcasestr(path, ".ico") != NULL)
            return "image/x-icon";
        if (strcasestr(path, ".jpg") != NULL)
            return "image/jpeg";
        if (strcasestr(path, ".js") != NULL)
            return "text/javascript";
        if (strcasestr(path, ".php") != NULL)
            return "text/x-php";
        if (strcasestr(path, ".png") != NULL)
            return "image/png";
        else
            return NULL;
    }
...
{
    if (strncmp(line, "GET ", sizeof(char) * 4) != 0)
    {
        error(405);
        return false;
    }
    
    const char* buff1 = strchr(line, ' ') + sizeof(char);
    const char* buff2 = NULL;
    ...
    {
        for (int i = 0, len = strlen(buff1); i < len; i++)
        {
            if (buff1[i] == ' ')
            {
                abs_path[i] = '\0';
            ...
            else if (buff1[i] == '?')
            {
                ...
                {
                    for (int j = 0, len1 = strlen(buff3); j < len1; j++)
                    {
                        if (buff3[j] == ' ')
                        {
                            query[j] = '\0';
